...
import { Camera, CloudRain, LogOut, MapPin, Mic, Sparkles, Video } from "lucide-react";
    ...
    <main className="relative min-h-dvh px-5 pb-28 pt-5">
      <header className="mb-6 flex items-center justify-between">
        ...
        <button
          ...
          className="rounded-full p-2 text-m3-onSurface/45"
        >
      ...
      >
        ...
        <Link href="/field-scan">
          <motion.button
            type="button"
            whileTap={{ scale: 0.97 }}
            className="flex w-full items-center justify-center gap-3 rounded-5xl bg-g-blue py-4 text-base font-bold text-white shadow-float"
          >
            <Video className="h-5 w-5" />
            {language === "rw" ? "Scan umurima (Video)" : "Field Video Sweep"}
          </motion.button>
        </Link>
